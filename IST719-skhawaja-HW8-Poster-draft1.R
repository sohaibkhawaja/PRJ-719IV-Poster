...
#This is an added line to test the git push
...
# tables: studies Browse_Conditions Browse_Interventions
...
s.raw <- data.frame(dbGetQuery(con, 
...
s.unk <- dbGetQuery(con, "SELECT nct_id, start_date, completion_date, primary_completion_date, study_type, acronym, brief_title, overall_status, phase, source, is_fda_regulated_drug, is_fda_regulated_device 
                          FROM studies
                          WHERE overall_status IN ('Unknown status')")

condition_count <- dbGetQuery(con, "SELECT COUNT(*)
...
#setwd("C:/DataScience/PRJ-719IV/HW8-FinalPoster/data")
